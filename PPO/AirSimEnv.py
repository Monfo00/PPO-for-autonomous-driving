...
class AirsimEnv(gym.Env):
   ...
   # setting connection with AirSim simulator
    ...
    def _init_reward_points(self):
        road_points = []
        with open('.../final_reward_points.csv', newline="") as csvfile:
    ...
    def step(self, action, j):
        ...
        car_state = self.client.getCarState()
        reward, out_street = self.compute_reward(car_state, info_collision, j)

        # check if the episode is done
        done = self.is_done(car_state, info_collision, out_street, j)
    ...
    def compute_reward(self, car_state, info_collision, j, evaluation=False):
        ...
        #print(f"Distanza calcolata:{distance}")
    ...
    def is_done(self, car_state, info_collision, out_street, j):

        speed = car_state.speed

        return 1 if info_collision.has_collided or out_street or (speed < 2 and j>25) or j>2000 else 0
